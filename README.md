...
`.env` must never be committed. Once this directory is under version
control, confirm `.env` is listed in `.gitignore` before your first commit.
...
This directory is its own git repository. Two rules matter:
`.state/<host>.yaml` must be committed — losing it means the next `apply`
cannot recognize existing entities and recreates everything as duplicates —
and `.env` must never be, since it holds the API key and the Basic auth
credentials. `.gitignore` covers `.env`; check it is still listed before
